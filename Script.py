class script(object):
    START_TXT = """👋 <b>ʜᴇʏ ʙᴀsᴛᴀʀᴅ ,</b> {}<b>,</b>
<b>ɪ'ᴍ </b><a href=https://t.me/{}>{},</a> <b>ʏᴏᴜ ᴍᴀʏ ᴋɴᴏᴡ ᴍᴇ.</b>

<b>● ɪ ᴀᴍ ᴀ ɢʀᴇᴀᴛ ʙᴀʙʏsɪᴛᴛᴇʀ ғᴏʀ ʏᴏᴜʀ ғɪʟᴇs.
● ᴊᴜsᴛ ᴀᴅᴅ ᴍᴇ ᴛᴏ ʏᴏᴜʀ ɢʀᴏᴜᴘ ᴀɴᴅ ᴍᴀᴋᴇ ᴍᴇ ᴀᴅᴍɪɴ ᴛʜᴇʀᴇ.
● ᴛʜᴇɴ ʏᴏᴜ ᴄᴀɴ sᴇᴇ ᴍʏ ᴡʜᴏʟᴇsᴏᴍᴇ ᴘᴏᴡᴇʀs...👑</b>

😎 <b>ᴍʏ ғʀɪᴇɴᴅ :</b> <a href=https://t.me/themastertheblaster>❣️ ∂εcσ∂εɾ รεεɱαɳ ❣️</a></b>"""

    HELP_TXT = """<b>ʜᴇʏ ʙᴜᴅᴅʏ {}
ʜᴇʀᴇ ɪs ᴍʏ ʜᴇʟᴘ ᴄᴏᴍᴍᴀɴᴅs...</b>"""
    ABOUT_TXT = """<b>😎 ᴍʏ ɴᴀᴍᴇ : <a href=https://t.me/mkvstevefilter_bot>💔 ѕτєνє нαяяιиgτοи 💔</a></b>
<b>🫂 ᴍʏ ғʀɪᴇɴᴅ : <a href=https://t.me/themastertheblaster>❣️ ∂εcσ∂εɾ รεεɱαɳ ❣️</a></b>
<b>🇵🇷 ᴄᴏᴜɴᴛʀʏ :  <a href=https://www.google.com/search?q=USA+&ei=Z3rKYvOXGtS34t4Pp6mEgAU&ved=0ahUKEwiz5_KR4e34AhXUm9gFHacUAVAQ4dUDCA4&uact=5&oq=USA+&gs_lcp=_OmgCcAF4A4AB3ASIAd8dkgEHMi0yLjUtNpgBAKABAbABCsgBEMABAdoBBggBEAEYCdoBBggCEAEYCA&sclient=gws-wiz#>ᴜsᴀ</a></b>
<b>🔊 ʟᴀɴɢᴜᴀɢᴇ : <a href=https://en.wikipedia.org/wiki/English_language>ᴇɴɢʟɪsʜ</a></b>
<b>🗽 sᴛᴀᴛᴇ : <a href=https://www.google.com/search?q=texas&ei=uozKYoTTHvjNseMP9P-Y6A0&gs_ssp=eJzj4tDP1TcwT4rPMWD0Yi1JrUgsBgAqsgUX&oq=texas&gs_lcp=&sclient=gws-wiz#>ᴛᴇxᴀs</a></b>
<b>🪴 ʙᴏʀɴ : <a href=https://www.google.com/search?q=hawkins+city&ei=1ozKYvWeL8KUseMPkNefwAc&gs_ssp=eJzj4tTP1TcwNEhONjRg9OLJSCzPzswrVkjOLKkEAFl_B7w&oq=haw&gs_lcp=&sclient=gws-wiz#>ʜᴀᴡᴋɪɴs</a></b>
<b>❤️‍🔥 ᴍʏ ʟᴏᴠᴇʀ : <a href=https://www.google.com/search?q=nancy+wheeler&ei=Ro3KYqShHe-hseMP5ZuS2Ak&oq=NAn&gs_lcp=&sclient=gws-wiz>💌 ɴᴀɴᴄʏ ᴡʜᴇᴇʟᴇʀ 💌</a></b>
<b>🏢 ᴍʏ ᴏғғɪᴄᴇ : <a href=https://t.me/Mkv_blasters>👑 ᴍᴋᴠ ʙʟᴀsᴛᴇʀs 👑™</a></b>
<b>👨‍🏫 ᴍʏ sᴄʜᴏᴏʟ : <a href=https://www.google.com/search?q=hawkins+high+school&oq=hawkins+hi&aqs=chrome.0.0i131i433i512j69i57j0i512l7.6697j0j9&client=ms-android-samsung-ss&sourceid=chrome-mobile&ie=UTF-8#imgrc=KF1BErh5KE74eM>ʜᴀᴡᴋɪɴs ʜɪɢʜ sᴄʜᴏᴏʟ</a></b>"""
    DONATION_TXT = """<b>𝐂𝐨𝐨𝐤𝐢𝐞𝐬 & 𝐒𝐦𝐨𝐨𝐭𝐡𝐢𝐞𝐬</b> 

›› <b>𝐃𝐨𝐧𝐚𝐭𝐢𝐨𝐧</b>

⪼ <b>𝐘𝐨𝐮 𝐂𝐚𝐧 𝐁𝐮𝐲 𝐌𝐞 𝐀𝐧𝐲 𝐂𝐨𝐨𝐤𝐢𝐞 (𝐨𝐫) 𝐒𝐦𝐨𝐨𝐭𝐡𝐢𝐞 💳. 
<b>━━━━━━━━━᚜ Payment Methods ᚛━━━━━━━━━
✮ 𝗚𝗼𝗼𝗴𝗹𝗲𝗣𝗮𝘆
✮ 𝗣𝗮𝘆𝘁𝗺
✮ 𝗣𝗵𝗼𝗻𝗲𝗣𝗲
✮ 𝗣𝗮𝘆𝗣𝗮𝗹
_𝐂𝐨𝐧𝐭𝐚𝐜𝐭 𝐌𝐞 𝐅𝐨𝐫 𝐊𝐧𝐨𝐰 𝐀𝐛𝐨𝐮𝐭 𝐓𝐡𝐞 𝐏𝐚𝐲𝐦𝐞𝐧𝐭 𝐈𝐧𝐟𝐨_
━━━━━━━━━━━━᚜ <a href=https://t.me/themastertheblaster><b>❣️ ∂εcσ∂εɾ รεεɱαɳ ❣️</b></a> ᚛━━━━━━━━━━━━

›› <b>𝐏𝐚𝐢𝐝 𝐏𝐫𝐨𝐦𝐨𝐭𝐢𝐨𝐧</b>

⪼ <b>𝐂𝐨𝐧𝐭𝐚𝐜𝐭 𝐌𝐞 𝐖𝐢𝐭𝐡 𝐘𝐨𝐮 𝐂𝐨𝐧𝐭𝐞𝐧𝐭 𝐖𝐡𝐢𝐜𝐡 𝐘𝐨𝐮 𝐖𝐚𝐧𝐭 𝐓𝐨 𝐏𝐫𝐨𝐦𝐨𝐭𝐞 . 
<b>━━━━━━━━━᚜ Payment Methods ᚛━━━━━━━━━
✮ 𝗚𝗼𝗼𝗴𝗹𝗲𝗣𝗮𝘆
✮ 𝗣𝗮𝘆𝘁𝗺
✮ 𝗣𝗵𝗼𝗻𝗲𝗣𝗲
✮ 𝗣𝗮𝘆𝗣𝗮𝗹
_𝐂𝐨𝐧𝐭𝐚𝐜𝐭 𝐌𝐞 𝐖𝐢𝐭𝐡 𝐘𝐨𝐮𝐫 𝐂𝐨𝐧𝐭𝐞𝐧𝐭 𝐀𝐧𝐝 𝐊𝐧𝐨𝐰 𝐀𝐛𝐨𝐮𝐭 𝐓𝐡𝐞 𝐏𝐚𝐲𝐦𝐞𝐧𝐭 𝐈𝐧𝐟𝐨_
━━━━━━━━━━━━᚜ <a href=https://t.me/themastertheblaster><b>❣️ ∂εcσ∂εɾ รεεɱαɳ ❣️</b></a> ᚛━━━━━━━━━━━━"""
    PROMOTION_TXT = """<b>〄 𝐏𝐚𝐢𝐝 𝐏𝐫𝐨𝐦𝐨𝐭𝐢𝐨𝐧 〄</b>

⪼ <b>𝐂𝐨𝐧𝐭𝐚𝐜𝐭 𝐌𝐞 𝐖𝐢𝐭𝐡 𝐘𝐨𝐮 𝐂𝐨𝐧𝐭𝐞𝐧𝐭 𝐖𝐡𝐢𝐜𝐡 𝐘𝐨𝐮 𝐖𝐚𝐧𝐭 𝐓𝐨 𝐏𝐫𝐨𝐦𝐨𝐭𝐞 . 
<b>━━━━━━━━━᚜ Payment Methods ᚛━━━━━━━━━
✮ 𝗚𝗼𝗼𝗴𝗹𝗲𝗣𝗮𝘆
✮ 𝗣𝗮𝘆𝘁𝗺
✮ 𝗣𝗵𝗼𝗻𝗲𝗣𝗲
✮ 𝗣𝗮𝘆𝗣𝗮𝗹
_𝐂𝐨𝐧𝐭𝐚𝐜𝐭 𝐌𝐞 𝐖𝐢𝐭𝐡 𝐘𝐨𝐮𝐫 𝐂𝐨𝐧𝐭𝐞𝐧𝐭 𝐀𝐧𝐝 𝐊𝐧𝐨𝐰 𝐀𝐛𝐨𝐮𝐭 𝐓𝐡𝐞 𝐏𝐚𝐲𝐦𝐞𝐧𝐭 𝐈𝐧𝐟𝐨_
━━━━━━━━━━━━᚜ <a href=https://t.me/themastertheblaster><b>❣️ ∂εcσ∂εɾ รεεɱαɳ ❣️</b></a> ᚛━━━━━━━━━━━━""" 
    FILE_TXT = """<b>● ʜᴇʟᴘ : ғɪʟᴇ sᴛᴏʀᴇ ᴍᴏᴅᴜʟᴇ</b>

<b>ʙʏ ᴜsɪɴɢ ᴛʜɪs ᴍᴏᴅᴜʟᴇ ʏᴏᴜ ᴄᴀɴ sᴛᴏʀᴇ ғɪʟᴇs ɪɴ ᴍʏ ᴅᴀᴛᴀʙᴀsᴇ ᴀɴᴅ ɪʟʟ ɢɪᴠᴇ ʏᴏᴜ ᴀ ᴘᴇʀᴍᴀɴᴇɴᴛ ʟɪɴᴋ ᴛᴏ ᴀᴄᴄᴇss ᴛʜᴇ sᴀᴠᴇᴅ ғɪʟᴇs.ɪғ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ᴀᴅᴅ ғɪʟᴇs ғʀᴏᴍ ᴛʜᴇ ᴘᴜʙʟɪᴄ ᴄʜᴀɴɴᴇʟ ᴛʜᴇɴ sᴇɴᴅ ᴍᴇ ᴛʜᴇ ғɪʟᴇ ʟɪɴᴋ ᴏɴʟʏ ᴏʀ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ᴀᴅᴅ ғɪʟᴇs ғʀᴏᴍ ᴘʀɪᴠᴀᴛᴇ ᴄʜᴀɴɴᴇʟ ʏᴏᴜ ᴍᴜsᴛ ᴍᴀᴋᴇ ᴍᴇ ᴀᴅᴍɪɴ ᴏɴ ᴛʜᴇ ᴄʜᴀɴɴᴇʟ ᴛᴏ ᴀᴄᴄᴇss ғɪʟᴇs... </b>

●<b> ᴄᴏᴍᴍᴀɴᴅs ᴀɴᴅ ᴜsᴀɢᴇ ›</b>

➪ <b>/plink</b> <b>›› ʀᴇᴘʟʏ ᴛᴏ ᴀɴʏ ᴍᴇᴅɪᴀ ᴛᴏ ɢᴇᴛ ʟɪɴᴋ</b>
➪ <b>/pbatch</b> <b>›› ᴜsᴇ ʏᴏᴜʀ ᴍᴇᴅɪᴀ ʟɪɴᴋ ᴡɪᴛʜ ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ</b>
➪ <b>/batch</b> <b>›› ᴛᴏ ᴄʀᴇᴀᴛᴇ ʟɪɴᴋ ғᴏʀ ᴍᴜʟᴛɪᴘʟᴇ ғɪʟᴇs.</b>

● <b>ᴇxᴀᴍᴘʟᴇ  ››</b>

<code>/batch https://t.me/mkv_blasters/1 https://t.me/mkv_blasters/5</code>

● <b>ᴀʟʟ ᴄʀᴇᴅɪᴛs :</b> <a href=https://t.me/Mkv_bots><b>🤖 ᴍᴋᴠ ʙᴏᴛs 🤖</b></a>"""
    WHOIS_TXT ="""● <b>ᴡʜᴏɪs ᴍᴏᴅᴜʟᴇ ››</b> 

<b>ɴᴏᴛᴇ : ɢɪᴠᴇ ᴍᴇ ᴀ ᴜsᴇʀ ғᴜʟʟ ᴅᴇᴛᴀɪʟ</b>

➪ <b>/whois</b><b> : ɢɪᴠᴇ ᴍᴇ ᴀ ᴜsᴇʀ ғᴜʟʟ ᴅᴇᴛᴀɪʟ...</b>

<b>● ᴀʟʟ ᴄʀᴇᴅɪᴛs :</b> <a href=https://t.me/Mkv_bots><b>🤖 ᴍᴋᴠ ʙᴏᴛs 🤖</b></a>"""
    FUN_TXT ="""<b>Gᴀᴍᴇs ››</b>

<b>⚡ᴊᴜsᴛ sᴏᴍᴇ ᴋɪɴᴅ ᴏғ ғᴜɴ ᴛʜɪɴɢs ⚡</b>

➪ <b>/Dice</b> <b>-</b> <b>ʀᴏʟᴇ ᴛʜᴇ ᴅɪᴄᴇ</b>
➪ <b>/Throw</b> <b>ᴏʀ</b> b>/Dart</b> <b>-</b> <b>ᴛᴏ ᴍᴀᴋᴇ ᴅᴀʀᴛ</b>
➪ <b>/Runs</b> <b>-</b> <b>sᴏᴍᴇ ʀᴀɴᴅᴏᴍ ᴅɪᴀʟᴏɢᴜᴇs</b>
➪ <b>/Goal</b> <b>ᴏʀ</b> <b>/Shoot</b> <b>-</b> <b>ᴛᴏ ᴍᴀᴋᴇ ᴀ ɢᴏᴀʟ ᴏʀ sʜᴏᴏᴛ</b>
➪ <b>/Luck</b> <b>ᴏʀ</b> <b>/Cownd</b> <b>-</b> <b>sᴘɪɴ ᴀɴᴅ ᴛʀʏ ᴛᴏ ʏᴏᴜʀ ʟᴜᴄᴋ </b>

● <b>ᴀʟʟ ᴄʀᴇᴅɪᴛs :</b> <a href=https://t.me/Mkv_bots><b>🤖 ᴍᴋᴠ ʙᴏᴛs 🤖</b></a>"""
    SOURCE_TXT = """
<b>ᴠᴀᴀ ᴀʀᴜɴᴀᴀᴄʜᴀʟᴏ ɴᴇᴇ ᴠᴀʀᴜᴠᴇɴɴᴜ ᴛʜᴇʀɪʏᴜ</b>"""
    DEPLOY_TXT = """<b>ʜᴏᴡ ᴛᴏ ʙᴇᴄᴏᴍᴇ ᴍʏ ʙᴇsᴛ ғʀɪᴇɴᴅ...?</b> 
  
<b>● ᴛᴜᴛᴏʀɪᴀʟ ››</b> <i><b>https://youtube.com/channel/UCdk53DCXKoKOKgKeohUEnmA</b></i>

<b>ɪғ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ʙᴇᴄᴏᴍᴇ ᴍʏ ʙᴇsᴛ ғʀɪᴇɴᴅ <a href=https://t.me/themastertheblaster>❣️ ∂εcσ∂εɾ รεεɱαɳ ❣️</a></b>

● <b>ᴀʟʟ ᴄʀᴇᴅɪᴛs :</b> <a href=https://t.me/Mkv_bots><b>🤖 ᴍᴋᴠ ʙᴏᴛs 🤖</b></a>"""
    MANUELFILTER_TXT = """<b>● ʜᴇʟᴘ :  ᴍᴀɴᴜᴀʟ ғɪʟᴛᴇʀ</b>

<b>ᴍᴀɴᴜᴀʟ ғɪʟᴛᴇʀ ɪs ᴛʜᴇ ғᴇᴀᴛᴜʀᴇ ᴡᴇʀᴇ ᴜsᴇʀs ᴄᴀɴ sᴇᴛ ᴀᴜᴛᴏᴍᴀᴛᴇᴅ ʀᴇᴘʟɪᴇs ғᴏʀ ᴀ ᴘᴀʀᴛɪᴄᴜʟᴀʀ ᴋᴇʏᴡᴏʀᴅ ᴀɴᴅ</b> <a href=https://t.me/Mkvstevefilter_bot><b>ѕτєνє</b></a>  <b>ᴡɪʟʟ ʀᴇsᴘᴏɴᴅ ᴡʜᴇɴᴇᴠᴇʀ ᴀ ᴋᴇʏᴡᴏʀᴅ ɪs ғᴏᴜɴᴅ ᴛʜᴇ ᴍᴇssᴀɢᴇ ᴡɪʟʟ ʀᴇᴘʟɪᴇᴅ...</b>

<b>● ɴᴏᴛᴇ ››</b>

➪ <a href=https://t.me/Mkvstevefilter_bot><b>ѕτєνє</b></a> <b>sʜᴏᴜʟᴅ ʜᴀᴠᴇ ᴀᴅᴍɪɴ ᴘʀɪᴠɪʟʟᴀɢᴇ.</b>
➪<b> ᴏɴʟʏ ᴀᴅᴍɪɴs ᴄᴀɴ ᴀᴅᴅ ғɪʟᴛᴇʀs ɪɴ ᴀ ᴄʜᴀᴛ.
➪ ᴀʟᴇʀᴛ ʙᴜᴛᴛᴏɴs ʜᴀᴠᴇ ᴀ ʟɪᴍɪᴛ ᴏғ 64 ᴄʜᴀʀᴀᴄᴛᴇʀs.</b>

<b>● ᴄᴏᴍᴍᴀɴᴅs ᴀɴᴅ ᴜsᴀɢᴇ ››</b>

➪ <b>/filter</b> <b>- ᴀᴅᴅ ᴀ ғɪʟᴛᴇʀ ɪɴ ᴄʜᴀᴛ</b>
➪ <b>/filters</b> <b>- ʟɪsᴛ ᴀʟʟ ᴛʜᴇ ғɪʟᴛᴇʀs ᴏғ ᴀ ᴄʜᴀᴛ</b>
➪ <b>/del</b> <b>- ᴅᴇʟᴇᴛᴇ ᴀ sᴘᴇᴄɪғɪᴄ ғɪʟᴛᴇʀ ɪɴ ᴄʜᴀᴛ</b>
➪ <b>/delall</b> <b>- ᴅᴇʟᴇᴛᴇ ᴛʜᴇ ᴡʜᴏʟᴇ ғɪʟᴛᴇʀs ɪɴ ᴀ ᴄʜᴀᴛ (ᴄʜᴀᴛ ᴏᴡɴᴇʀ ᴏɴʟʏ)</b>

<b>● ᴀʟʟ ᴄʀᴇᴅɪᴛs :</b> <a href=https://t.me/Mkv_bots><b>🤖 ᴍᴋᴠ ʙᴏᴛs 🤖</b></a>"""
    SONG_TXT = """<b>● ʜᴇʟᴘ :  sᴏɴɢ ᴅᴏᴡɴʟᴏᴀᴅ ᴍᴏᴅᴜʟᴇ </b>

<b>sᴏɴɢ ᴅᴏᴡɴʟᴏᴀᴅ ᴍᴏᴅᴜʟᴇ ɪs ᴀᴠᴀɪʟᴀʙʟᴇ ғᴏʀ ᴛʜᴏsᴇ ᴡʜᴏ ʟᴏᴠᴇ ᴍᴜsɪᴄ...ʏᴏᴜ ᴄᴀɴ ᴜsᴇ ᴛʜɪs ғᴇᴀᴛᴜʀᴇ ғᴏʀ ᴅᴏᴡɴʟᴏᴀᴅ ᴀɴʏ sᴏɴɢ ᴡɪᴛʜ sᴜᴘᴇʀ ғᴀsᴛ sᴘᴇᴇᴅ... ᴡᴏʀᴋs ᴏɴʟʏ ᴏɴ ɢʀᴏᴜᴘs...</b>

<b>● ᴄᴏᴍᴍᴀɴᴅs ››</b>

➪ <b>/song - sᴏɴɢ ɴᴀᴍᴇ ( ᴡᴏʀᴋs ᴏɴʟʏ ᴏɴ ɢʀᴏᴜᴘs )</b>

<b>● ᴀʟʟ ᴄʀᴇᴅɪᴛs :</b> <a href=https://t.me/Mkv_bots><b>🤖 ᴍᴋᴠ ʙᴏᴛs 🤖</b></a>"""
    PIN_TXT ="""<b>● ʜᴇʟᴘ : ᴘɪɴ ᴍᴏᴅᴜʟᴇ </b>

<b>ɪᴛ ɪs ᴀᴠᴀɪʟᴀʙʟᴇ ғᴏʀ ᴘɪɴɴɪɴɢ ᴀ ᴍᴇssᴀɢᴇ ɪɴ ᴛʜᴇ ɢʀᴏᴜᴘ.</b>

<b>ᴀʟʟ ᴛʜᴇ ᴘɪɴ ʀᴇʟᴀᴛᴇᴅ ᴄᴏᴍᴍᴀɴᴅs ᴄᴀɴ ʙᴇ ғᴏᴜɴᴅ ʜᴇʀᴇ...</b>

<b>● ᴄᴏᴍᴍᴀɴᴅs ᴀɴᴅ ᴜsᴀɢᴇ ››</b>

➪ <b>/pin - ᴛᴏ ᴘɪɴ ᴛʜᴇ ᴍᴇssᴀɢᴇ ᴏɴ ʏᴏᴜʀ ᴄʜᴀᴛs
➪ /unpin - ᴛᴏ ᴜɴᴘɪɴ ᴛʜᴇ ᴄᴜʀʀᴇɴᴛ ᴘɪɴɴᴇᴅ ᴍᴇssᴀɢᴇ </b>

<b>● ᴀʟʟ ᴄʀᴇᴅɪᴛs :</b> <a href=https://t.me/Mkv_bots><b>🤖 ᴍᴋᴠ ʙᴏᴛs 🤖</b></a>"""
    PASTE_TXT = """<b>● ʜᴇʟᴘ : ᴘᴀsᴛᴇ

ᴘᴀsᴛᴇ sᴏᴍᴇ ᴛᴇxᴛs ᴏʀ ᴅᴏᴄᴜᴍᴇɴᴛs ᴏɴ ᴀ ᴡᴇʙsɪᴛᴇ !</b>

<b>● ᴄᴏᴍᴍᴀɴᴅs ᴀɴᴅ ᴜsᴀɢᴇ ››</b>

➪ <b>/paste [ᴛᴇxᴛ] - ᴘᴀsᴛᴇ ᴛʜᴇ ɢɪᴠᴇɴ ᴛᴇxᴛ ᴏɴ ᴘᴀsᴛʏ</b>

<b>● ɴᴏᴛᴇ ››</b>

➪ <b>ᴛʜᴇsᴇ ᴄᴏᴍᴍᴀɴᴅs ᴡᴏʀᴋs ᴏɴ ʙᴏᴛʜ ᴘᴍ ᴀɴᴅ ɢʀᴏᴜᴘ.</b>
➪ <b>ᴛʜᴇsᴇ ᴄᴏᴍᴍᴀɴᴅs ᴄᴀɴ ʙᴇ ᴜsᴇᴅ ʙʏ ᴀɴʏ ɢʀᴏᴜᴘ ᴍᴇᴍʙᴇʀ.</b>

<b>● ᴀʟʟ ᴄʀᴇᴅɪᴛs :</b> <a href=https://t.me/Mkv_bots><b>🤖 ᴍᴋᴠ ʙᴏᴛs 🤖</b></a>"""
    TTS_TXT = """<b>● ʜᴇʟᴘ :  ᴛᴛs ᴍᴏᴅᴜʟᴇ

ᴛʀᴀɴsʟᴀᴛᴇ ᴛᴇxᴛ ᴛᴏ sᴘᴇᴇᴄʜ</b>

<b>● ᴄᴏᴍᴍᴀɴᴅs ᴀɴᴅ ᴜsᴀɢᴇ ››</b>

➪ <b>/tts : ᴄᴏɴᴠᴇʀᴛ ᴛᴇxᴛ ᴛᴏ sᴘᴇᴇᴄʜ</b>

<b>● ɴᴏᴛᴇ ››</b>

➪ <b>ɪᴍᴅʙ sʜᴏᴜʟᴅ ʜᴀᴠᴇ ᴀᴅᴍɪɴ ᴘʀɪᴠɪʟʟᴀɢᴇ.</b>
➪ <b>ᴛʜᴇsᴇ ᴄᴏᴍᴍᴀɴᴅs ᴡᴏʀᴋs ᴏɴ ʙᴏᴛʜ ᴘᴍ ᴀɴᴅ ɢʀᴏᴜᴘ.</b>
➪ <b>ɪᴍᴅʙ ᴄᴀɴ ᴛʀᴀɴsʟᴀᴛᴇ ᴛᴇxᴛs ᴛᴏ 200+ ʟᴀɴɢᴜᴀɢᴇs.</b>

<b>● ᴀʟʟ ᴄʀᴇᴅɪᴛs :</b> <a href=https://t.me/Mkv_bots><b>🤖 ᴍᴋᴠ ʙᴏᴛs 🤖</b></a>"""
    PINGS_TXT ="""<b>● ʜᴇʟᴘ : ᴘɪɴɢ 

ʜᴇʟᴘs ʏᴏᴜ ᴛᴏ ᴋɴᴏᴡ ʏᴏᴜʀ ᴘɪɴɢ </b>

<b>● ᴄᴏᴍᴍᴀɴᴅs ››</b>

➪ <b>/alive - ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜ ᴀʀᴇ ᴀʟɪᴠᴇ.</b>
➪ <b>/help - ᴛᴏ ɢᴇᴛ ʜᴇʟᴘ.</b>
➪ <b>/ping - ᴛᴏ ɢᴇᴛ ʏᴏᴜʀ ᴘɪɴɢ.</b>
➪ <b>/repo - sᴏᴜʀᴄᴇ ᴄᴏᴅᴇ.</b>
➪<b> /channel - ᴄʜᴀɴɴᴇʟ ᴅᴇᴛᴀɪʟs.</b>
➪ <b>/steve - ʙᴏᴛ ʟɪɴᴋ.</b>

<b>● ᴜsᴀɢᴇ ››</b>

➪ <b>ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅs ᴄᴀɴ ʙᴇ ᴜsᴇᴅ ɪɴ ᴘᴍs ᴀɴᴅ ɢʀᴏᴜᴘs</b>
➪ <b>ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅs ᴄᴀɴ ʙᴇ ᴜsᴇᴅ ʙᴜʏ ᴇᴠᴇʀʏᴏɴᴇ ɪɴ ᴛʜᴇ ɢʀᴏᴜᴘs ᴀɴᴅ ʙᴏᴛs ᴘᴍ</b>
➪ <b>sʜᴀʀᴇ ᴜs ғᴏʀ ᴍᴏʀᴇ ғᴇᴀᴛᴜʀᴇs</b>

<b>● ᴀʟʟ ᴄʀᴇᴅɪᴛs :</b> <a href=https://t.me/Mkv_bots><b>🤖 ᴍᴋᴠ ʙᴏᴛs 🤖</b></a>"""
    TELE_TXT = """<b>● ʜᴇʟᴘ :  ᴛᴇʟᴇɢʀᴀᴘʜ

ᴅᴏ ᴀs ʏᴏᴜ ᴡɪsʜ ᴡɪᴛʜ ᴛᴇʟᴇɢʀᴀᴘʜ ᴍᴏᴅᴜʟᴇ !</b>

<b>● ᴜsᴀɢᴇ ››</b>

➪ <b>/telegraph - sᴇɴᴅ ᴍᴇ ᴘɪᴄᴛᴜʀᴇ ᴏʀ ᴠɪᴅᴇ ᴜɴᴅᴇʀ (5ᴍʙ)</b>

<b>● ɴᴏᴛᴇ ››</b>

➪ <b>ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ ɪs ᴀᴠᴀɪʟᴀʙʟᴇ ɪɴ ɢᴏᴜᴘs ᴀɴᴅ ᴘᴍs</b>
➪ <b>ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ ᴄᴀɴ ʙᴇ ᴜsᴇᴅ ʙʏ ᴇᴠᴇʀʏᴏɴᴇ</b>

<b>● ᴀʟʟ ᴄʀᴇᴅɪᴛs :</b> <a href=https://t.me/Mkv_bots><b>🤖 ᴍᴋᴠ ʙᴏᴛs 🤖</b></a>"""

    PRIVATEBOT_TXT = """<b>ᴅᴏ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ʙᴇᴄᴏᴍᴇ ᴍʏ ʙᴇsᴛ ғʀɪᴇɴᴅ...?

›› ᴅᴏ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ʙᴇ ᴍʏ ʙᴇsᴛ ғʀɪᴇɴᴅ ʟɪᴋᴇ</b> <a href=https://t.me/themastertheblaster><b>❣️ ∂εcσ∂εɾ ❣️</b></a>.
<b>›› ᴡɪᴛʜ ᴀʟʟ ʏᴏᴜʀ ᴄʀᴇᴅɪᴛs
›› ᴡɪᴛʜ ʏᴏᴜʀ ᴏᴡɴᴇʀsʜɪᴘ
›› ᴄᴏɴᴛᴀᴄᴛ ᴍᴇ </b><a href=https://t.me/themastertheblaster><b>❣️ ∂εcσ∂εɾ ❣️</b></a>.

<b>● ᴀʟʟ ᴄʀᴇᴅɪᴛs :</b> <a href=https://t.me/Mkv_bots><b>🤖 ᴍᴋᴠ ʙᴏᴛs 🤖</b></a>"""

    JSON_TXT ="""<b>● ʜᴇʟᴘ :  ᴊsᴏɴ

ʙᴏᴛ ʀᴇᴛᴜʀɴs ᴊsᴏɴ ғᴏʀ ᴀʟʟ ʀᴇᴘʟɪᴇᴅ ᴍᴇssᴀɢᴇs ᴡɪᴛʜ /json</b>

<b>● ғᴇᴀᴛᴜʀᴇs ››</b>

➪ <b>ᴍᴇssᴀɢᴇ ᴇᴅɪᴛᴛɪɴɢ ᴊsᴏɴ</b>
➪ <b>ᴘᴍ sᴜᴘᴘᴏʀᴛ</b>
➪ <b>ɢʀᴏᴜᴘ sᴜᴘᴘᴏʀᴛ</b>

<b>● ɴᴏᴛᴇ ››

ᴇᴠᴇʀʏᴏɴᴇ ᴄᴀɴ ᴜsᴇ ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ , ɪғ sᴘᴀᴍɪɴɢ ʜᴀᴘᴘᴇɴs ʙᴏᴛ ᴡɪʟʟ ᴀᴜᴛᴏᴍᴀᴛɪᴄᴀʟʟʏ ʙᴀɴ ʏᴏᴜ ғʀᴏᴍ ᴛʜᴇ ɢʀᴏᴜᴘ.</b>

<b>● ᴀʟʟ ᴄʀᴇᴅɪᴛs :</b> <a href=https://t.me/Mkv_bots><b>🤖 ᴍᴋᴠ ʙᴏᴛs 🤖</b></a>"""
    PURGE_TXT = """<b>● ʜᴇʟᴘ : ᴘᴜʀɢᴇ
    
ᴅᴇʟᴇᴛᴇ ᴀ ʟᴏᴛ ᴏғ ᴍᴇssᴀɢᴇs ғʀᴏᴍ ɢʀᴏᴜᴘs ! </b>
    
<b>● ᴀᴅᴍɪɴ ››</b>

➪<b> /purge - ᴅᴇʟᴇᴛᴇ ᴀʟʟ ᴍᴇssᴀɢᴇs ғʀᴏᴍ ᴛʜᴇ ʀᴇᴘʟɪᴇᴅ ᴛᴏ ᴍᴇssᴀɢᴇ, ᴛᴏ ᴛʜᴇ ᴄᴜʀʀᴇɴᴛ ᴍᴇssᴀɢᴇ</b>

<b>● ᴀʟʟ ᴄʀᴇᴅɪᴛs :</b> <a href=https://t.me/Mkv_bots><b>🤖 ᴍᴋᴠ ʙᴏᴛs 🤖</b></a>"""
    BUTTON_TXT = """<b>● ʜᴇʟᴘ :  ʙᴜᴛᴛᴏɴs</b>

<a href=https://t.me/Mkvstevefilter_bot><b>ѕτєνє</b></a> <b>sᴜᴘᴘᴏʀᴛs ʙᴏᴛʜ ᴜʀʟ ᴀɴᴅ ᴀʟᴇʀᴛ ɪɴʟɪɴᴇ ʙᴜᴛᴛᴏɴs.</b>

<b>● ɴᴏᴛᴇ ››</b>

➪<b> ᴛᴇʟᴇɢʀᴀᴍ ᴡɪʟʟ ɴᴏᴛ ᴀʟʟᴏᴡs ʏᴏᴜ ᴛᴏ sᴇɴᴅ ʙᴜᴛᴛᴏɴs ᴡɪᴛʜᴏᴜᴛ ᴀɴʏ ᴄᴏɴᴛᴇɴᴛ, sᴏ ᴄᴏɴᴛᴇɴᴛ ɪs ᴍᴀɴᴅᴀᴛᴏʀʏ.</b>
➪ <a href=https://t.me/Mkvstevefilter_bot><b>ѕτєνє</b></a> <b>sᴜᴘᴘᴏʀᴛs ʙᴜᴛᴛᴏɴs ᴡɪᴛʜ ᴀɴʏ ᴛᴇʟᴇɢʀᴀᴍ ᴍᴇᴅɪᴀ ᴛʏᴘᴇ.</b>
➪ <b>ʙᴜᴛᴛᴏɴs sʜᴏᴜʟᴅ ʙᴇ ᴘʀᴏᴘᴇʀʟʏ ᴘᴀʀsᴇᴅ ᴀs ᴍᴀʀᴋᴅᴏᴡɴ ғᴏʀᴍᴀᴛ</b>

<b>● ᴜʀʟ ʙᴜᴛᴛᴏɴs ››</b>
<code>[Button Text](buttonurl:https://t.me/mkv_blasters)</code>

<b>● ᴀʟᴇʀᴛ ʙᴜᴛᴛᴏɴs ››</b>
<code>[Button Text](buttonalert:This is an alert message)</code>

<b>● ᴀʟʟ ᴄʀᴇᴅɪᴛs :</b> <a href=https://t.me/Mkv_bots><b>🤖 ᴍᴋᴠ ʙᴏᴛs 🤖</b></a>"""
    AUTOFILTER_TXT = """<b>● ʜᴇʟᴘ : ᴀᴜᴛᴏғɪʟᴛᴇʀ ᴏɴ/ᴏғғ ᴍᴏᴅᴜʟᴇ 

ᴀᴜᴛᴏғɪʟᴛᴇʀ ɪs ᴛʜᴇ ғᴇᴀᴛᴜʀᴇ ᴛᴏ ғɪʟᴛᴇʀ ᴀɴᴅ sᴀᴠᴇ ᴛʜᴇ ғɪʟᴇs ᴀᴜᴛᴏᴍᴀᴛɪᴄᴀʟʟʏ ғʀᴏʏᴄʜᴀɴɴᴇʟ ᴛᴏ ɢʀᴏᴜᴘ. ʏᴏᴜ ᴄᴀɴ ᴜsᴇ ᴛʜᴇ ғᴏʟʟᴏᴡɪɴɢ ᴄᴏᴍᴍᴀɴᴅs ᴛᴏ ᴏɴ ᴀɴᴅ ᴏғғ ᴛʜᴇ ᴀᴜᴛᴏғɪʟᴛᴇʀ ɪɴ ʏᴏᴜʀ ɢʀᴏᴜᴘ....</b>

<b>● ᴄᴏᴍᴍᴀɴᴅs ››</b>

➪ <b>/autofilter ᴏɴ - ᴇɴᴀʙʟᴇ ᴀᴜᴛᴏ ғɪʟᴛᴇʀ ɪɴ ᴛʜᴇ ɢʀᴏᴜᴘ </b>
➪ <b>/autofilter ᴏғғ - ᴅɪsᴀʙʟᴇ ᴀᴜᴛᴏ ғɪʟᴛᴇʀ ɪɴ ᴛʜᴇ ɢʀᴏᴜᴘ</b>
➪ <b>/set_template - sᴇᴛ ᴄᴜsᴛᴏᴍ ɪᴍᴅʙ ᴛᴇᴍᴘʟᴀᴛᴇ ғᴏʀ ᴀᴜᴛᴏ ᴜᴛᴏ ғɪʟᴛᴇʀ </b>
➪ <b>/get_template - ɢᴇᴛ ᴄᴜʀʀᴇɴᴛ ɪᴍᴅʙ ᴛᴇᴍᴘʟᴀᴛᴇ ғᴏʀ ᴀᴜᴛᴏ ᴜᴛᴏ ғɪʟᴛᴇʀ</b>

<b>● ᴀʟʟ ᴄʀᴇᴅɪᴛs :</b> <a href=https://t.me/Mkv_bots><b>🤖 ᴍᴋᴠ ʙᴏᴛs 🤖</b></a>"""
    CONNECTION_TXT = """<b>● ʜᴇʟᴘ : ᴄᴏɴɴᴇᴄᴛɪᴏɴs

ᴜsᴇᴅ ᴛᴏ ᴄᴏɴɴᴇᴄᴛ ʙᴏᴛ ᴛᴏ ᴘᴍ ғᴏʀ ᴍᴀɴᴀɢɪɴɢ ғɪʟᴛᴇʀs 
ɪᴛ ʜᴇʟᴘs ᴛᴏ ᴀᴠᴏɪᴅ sᴘᴀᴍᴍɪɴɢ ɪɴ ɢʀᴏᴜᴘs.</b>

<b>● ɴᴏᴛᴇ ››</b>
➪<b> ᴏɴʟʏ ᴀᴅᴍɪɴs ᴄᴀɴ ᴀᴅᴅ ᴀ ᴄᴏɴɴᴇᴄᴛɪᴏɴ.</b>
➪<b> sᴇɴᴅ /connect ғᴏʀ ᴄᴏɴɴᴇᴄᴛɪɴɢ ᴍᴇ ᴛᴏ ᴜʀ ᴘᴍ</b>

<b>● ᴄᴏᴍᴍᴀɴᴅs ᴀɴᴅ ᴜsᴀɢᴇ ››</b>
➪ <b>/connect  - ᴄᴏɴɴᴇᴄᴛ ᴀ ᴘᴀʀᴛɪᴄᴜʟᴀʀ ᴄʜᴀᴛ ᴛᴏ ʏᴏᴜʀ ᴘᴍ</b>
➪ <b>/disconnect  - ᴅɪsᴄᴏɴɴᴇᴄᴛ ғʀᴏᴍ ᴀ ᴄʜᴀᴛ</b>
➪ <b>/connections - ʟɪsᴛ ᴀʟʟ ʏᴏᴜʀ ᴄᴏɴɴᴇᴄᴛɪᴏɴs</b>

<b>● ᴀʟʟ ᴄʀᴇᴅɪᴛs :</b> <a href=https://t.me/Mkv_bots><b>🤖 ᴍᴋᴠ ʙᴏᴛs 🤖</b></a>"""
    EXTRAMOD_TXT = """<b>● ʜᴇʟᴘ : ᴇxᴛʀᴀ ᴍᴏᴅᴜʟᴇs</b>

<b>● ɴᴏᴛᴇ ››</b>

<b>ᴛʜᴇsᴇ ᴀʀᴇ ᴛʜᴇ ᴇxᴛʀᴀ ғᴇᴀᴛᴜʀᴇs ᴏғ</b> <a href=https://t.me/Mkvstevefilter_bot><b>ѕτєνє</b></a>

<b>● ᴄᴏᴍᴍᴀɴᴅs ᴀɴᴅ ᴜsᴀɢᴇ ››</b>

➪ <b>/id</b> - <code>get id of a specifed user.</code>
➪ <b>/info</b> - <code>get information about a user.</code>
➪ <b>/imdb</b> - <code>get the film information from IMDb source.</code>
➪ <b>/search</b> - <code>get the film information from various sources.</code>

<b>● ᴀʟʟ ᴄʀᴇᴅɪᴛs :</b> <a href=https://t.me/Mkv_bots><b>🤖 ᴍᴋᴠ ʙᴏᴛs 🤖</b></a>"""
    ADMIN_TXT = """<b>● ʜᴇʟᴘ : ᴀᴅᴍɪɴ ᴄᴏᴍᴍᴀɴᴅs </b>

<b>● ɴᴏᴛᴇ ››</b>

<b>ᴛʜɪs ᴍᴏᴅᴜʟᴇ ᴏɴʟʏ ᴡᴏʀᴋs ғᴏʀ ᴍʏ ᴀᴅᴍɪɴs<b>

<b>● ᴄᴏᴍᴍᴀɴᴅs ᴀɴᴅ ᴜsᴀɢᴇ ››</b>

➪ <b>/logs</b> - <code>to get the rescent errors</code>
➪ <b>/stats</b> - <code>to get status of files in db.</code>
➪ <b>/delete</b> - <code>to delete a specific file from db.</code>
➪ <b>/users</b> - <code>to get list of my users and ids.</code>
➪ <b>/chats</b> - <code>to get list of the my chats and ids </code>
➪ <b>/leave</b>  - <code>to leave from a chat.</code>
➪ <b>/disable</b>  -  <code>do disable a chat.</code>
➪ <b>/ban_user</b> - <code>to ban a user.</code>
➪ <b>/unban_user</b>  - <code>to unban a user.</code>
➪ <b>/channel</b> - <code>to get list of total connected channels</code>
➪ <b>/broadcast</b> - <code>to broadcast a message to all users</code>

<b>● ᴀʟʟ ᴄʀᴇᴅɪᴛs :</b> <a href=https://t.me/Mkv_bots><b>🤖 ᴍᴋᴠ ʙᴏᴛs 🤖</b></a>"""
    STATUS_TXT = """<b>📚 ᴛᴏᴛᴀʟ ғɪʟᴇs : <b>{}</b></b>
<b>👤 ᴛᴏᴛᴀʟ ᴜsᴇʀs : <b>{}</b></b>
<b>🪩 ᴛᴏᴛᴀʟ ᴄʜᴀᴛs : <b>{}</b></b>
<b>💾 ᴜsᴇᴅ sᴛᴏʀᴀɢᴇ : <b>{}</b></b>
<b>📀 ғʀᴇᴇ sᴛᴏʀᴀɢᴇ : <b>{}</b></b>"""
    LOG_TEXT_G = """<b>#ɴᴇᴡ ɢʀᴏᴜᴘ</b>
    
<b>🪩 ɢʀᴏᴜᴘ ⪼ {}(<code>{}</code>)</b>
<b>👤 ᴛᴏᴛᴀʟ ᴍᴇᴍʙᴇʀs ⪼ <code>{}</code></b>
<b>🎯 ᴀᴅᴅᴇᴅ ʙʏ ⪼ {}</b>
"""
    LOG_TEXT_P = """<b>#ɴᴇᴡ ᴜsᴇʀ</b>
    
<b>🪪 ɪᴅ - <code>{}</code></b>
<b>🔮 ɴᴀᴍᴇ - {}</b>
"""
    REPORT_TXT = """<b>● ʜᴇʟᴘ :  ʀᴇᴘᴏʀᴛ 

ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ ʜᴇʟᴘs ʏᴏᴜ ᴛᴏ ʀᴇᴘᴏʀᴛ ᴀ ᴍᴇssᴀɢᴇ ᴏʀ ᴀ ᴜsᴇʀ ᴛᴏ ᴛʜᴇ ᴀᴅᴍɪɴs ᴏғ ᴛʜᴇ ʀᴇsᴘᴇᴄᴛɪᴠᴇ ɢʀᴏᴜᴘ. ᴅᴏɴ'ᴛ ᴍɪsᴜsᴇ ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ.</b>

<b>● ᴄᴏᴍᴍᴀɴᴅs ᴀɴᴅ ᴜsᴀɢᴇ ››</b>

 ➪<b> /report 𝗈𝗋 @admins - ᴛᴏ ʀᴇᴘᴏʀᴛ ᴀ ᴜsᴇʀ ᴛᴏ ᴛʜᴇ ᴀᴅᴍɪɴs ( ʀᴇᴘʟʏ ᴛᴏ ᴀ ᴍᴇssᴀɢᴇ )</b>

<b>● ᴀʟʟ ᴄʀᴇᴅɪᴛs :</b> <a href=https://t.me/Mkv_bots><b>🤖 ᴍᴋᴠ ʙᴏᴛs 🤖</b></a>"""

    CORONA_TXT = """<b>● ʜᴇʟᴘ :  ᴄᴏᴠɪᴅ 

ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ ʜᴇʟᴘs ʏᴏᴜ ᴛᴏ ᴋɴᴏᴡ ᴅᴀɪʟʏ ɪɴғᴏʀᴍᴀᴛɪᴏɴ ᴀʙᴏᴜᴛ ᴄᴏᴠɪᴅ</b>


<b>● ᴄᴏᴍᴍᴀɴᴅs ᴀɴᴅ ᴜsᴀɢᴇ ››</b>


➪<b> /covid - 𝗎𝗌𝖾 𝗍𝗁𝗂𝗌 𝖼𝗈𝗆𝗆𝖺𝗇𝖽 𝗐𝗂𝗍𝗁 𝗒𝗈𝗎𝗋 𝖼𝗈𝗎𝗇𝗍𝗋𝗒 𝗇𝖺𝗆𝖾 𝗍𝗈 𝗀𝖾𝗍 𝖼𝗈𝗏𝗂𝖽 𝗂𝗇𝖿𝗈𝗋𝗆𝖺𝗍𝗂𝗈𝗇</b>

<b>● ᴇxᴀᴍᴘʟᴇ  ››</b>
<code>/covid 𝖨𝗇𝖽𝗂𝖺</code>

<b>● ᴀʟʟ ᴄʀᴇᴅɪᴛs :</b> <a href=https://t.me/Mkv_bots><b>🤖 ᴍᴋᴠ ʙᴏᴛs 🤖</b></a>"""

    URLSHORT_TXT = """<b>● ʜᴇʟᴘ :  ᴜʀʟ sʜᴏʀᴛɴᴇʀ

ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ ʜᴇʟᴘs ʏᴏᴜ ᴛᴏ sʜᴏʀᴛ ᴀ ᴜʀʟ</b>

<b>● ᴄᴏᴍᴍᴀɴᴅs ᴀɴᴅ ᴜsᴀɢᴇ ››</b>

➪<b> /short - ᴜsᴇ ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ ᴡɪᴛʜ ʏᴏᴜʀ ʟɪɴᴋ ᴛᴏ ɢᴇᴛ sʜᴏʀᴛᴇᴅ ʟɪɴᴋs</b>


<b>● ᴇxᴀᴍᴘʟᴇ  ››</b>
<code>/short https://youtube.com/channel/UCdk53DCXKoKOKgKeohUEnmA</code>

<b>● ᴀʟʟ ᴄʀᴇᴅɪᴛs :</b> <a href=https://t.me/Mkv_bots><b>🤖 ᴍᴋᴠ ʙᴏᴛs 🤖</b></a>"""

    VIDEO_TXT ="""<b>● ʜᴇʟᴘ :  ᴠɪᴅᴇᴏ ᴍᴏᴅᴜʟᴇ 

ʜᴇʟᴘ ʏᴏᴜ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅ ᴠɪᴅᴇᴏ ғʀᴏᴍ ʏᴏᴜᴛᴜʙᴇ </b>
 
<b>● ᴜsᴀɢᴇ ››</b>

<b>ʏᴏᴜ ᴄᴀɴ ᴅᴏᴡɴʟᴏᴀᴅ ᴀɴʏ ᴠɪᴅᴇᴏ ғʀᴏᴍ ʏᴏᴜᴛᴜʙᴇ </b>

<b>● ᴄᴏᴍᴍᴀɴᴅs ››</b>

➪ <b>ᴛʏᴘᴇ /video ᴏʀ /mp4 ᴀɴᴅ ( https://youtube.com/channel/UCdk53DCXKoKOKgKeohUEnmA )</b>

<b>● ᴇxᴀᴍᴘʟᴇ  ››</b>
<code>/video https://youtube.com/channel/UCdk53DCXKoKOKgKeohUEnmA</code>

<code>/mp4 https://youtube.com/channel/UCdk53DCXKoKOKgKeohUEnmA</code>

<b>● ᴀʟʟ ᴄʀᴇᴅɪᴛs :</b> <a href=https://t.me/Mkv_bots><b>🤖 ᴍᴋᴠ ʙᴏᴛs 🤖</b></a>"""

    ZOMBIES_TXT = """<b>● ʜᴇʟᴘ :  ᴋɪᴄᴋ ᴜsᴇʀs

ʜᴇʟᴘ ʏᴏᴜ ᴛᴏ ᴋɪᴄᴋ ᴜsᴇʀs. ᴋɪᴄᴋ ɪɴᴄᴀᴛɪᴠᴇ ᴍᴇᴍʙᴇʀs ғʀᴏᴍ ɢʀᴏᴜᴘ. ᴀᴅᴅ ᴍᴇ ᴀs ᴀᴅᴍɪɴ ᴡɪᴛʜ ʙᴀɴ ᴜsᴇʀs ᴘᴇʀᴍɪssɪᴏɴ ɪɴ ɢʀᴏᴜᴘ.</b>

<b>● ᴄᴏᴍᴍᴀɴᴅs ᴀɴᴅ ᴜsᴀɢᴇ ››</b>

➪<b> /inkick - ᴄᴏᴍᴍᴀɴᴅ ᴡɪᴛʜ ʀᴇϙᴜɪʀᴇᴅ ᴀʀɢᴜᴍᴇɴᴛs ᴀɴᴅ ɪ ᴡɪʟʟ ᴋɪᴄᴋ ᴍᴇᴍʙᴇʀs ғʀᴏᴍ ɢʀᴏᴜᴘ.</b>
➪<b> /instatus - ᴛᴏ ᴄʜᴇᴄᴋ ᴄᴜʀʀᴇɴᴛ sᴛᴀᴛᴜs ᴏғ ᴄʜᴀᴛ ᴍᴇᴍʙᴇʀ ғʀᴏᴍ ɢʀᴏᴜᴘ.</b>
➪<b> /inkick within_month long_time_ago - ᴛᴏ ᴋɪᴄᴋ ᴜsᴇʀs ᴡʜᴏ ᴀʀᴇ ᴏғғʟɪɴᴇ ғᴏʀ ᴍᴏʀᴇ ᴛʜᴀɴ 6-7 ᴅᴀʏs.</b>
➪<b> /inkick long_time_ago - ᴛᴏ ᴋɪᴄᴋ ᴍᴇᴍʙᴇʀs ᴡʜᴏ ᴀʀᴇ ᴏғғʟɪɴᴇ ғᴏʀ ᴍᴏʀᴇ ᴛʜᴀɴ ᴀ ᴍᴏɴᴛʜ ᴀɴᴅ ᴅᴇʟᴇᴛᴇᴅ ᴀᴄᴄᴏᴜɴᴛs.</b>
➪<b> /dkick - ᴛᴏ ᴋɪᴄᴋ ᴅᴇʟᴇᴛᴇᴅ ᴀᴄᴄᴏᴜɴᴛs.</b>

<b>● ᴀʟʟ ᴄʀᴇᴅɪᴛs :</b> <a href=https://t.me/Mkv_bots><b>🤖 ᴍᴋᴠ ʙᴏᴛs 🤖</b></a>"""

    IMAGE_TXT = """<b>● ʜᴇʟᴘ :  ɪᴍᴀɢᴇ

ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ ʜᴇʟᴘs ᴛᴏ ʏᴏᴜ ᴛᴏ ᴇᴅɪᴛ ɪᴍᴀɢᴇ ᴠᴇʀʏ ᴇᴀsɪʟʏ </b>


<b>● ᴄᴏᴍᴍᴀɴᴅs ᴀɴᴅ ᴜsᴀɢᴇ ››</b>

➪ <b>ᴊᴜsᴛ sᴇɴᴅ ᴍᴇ ᴀ ɪᴍᴀɢᴇ ᴛᴏ ᴇᴅɪᴛ</b>

<b>● ᴀʟʟ ᴄʀᴇᴅɪᴛs :</b> <a href=https://t.me/Mkv_bots><b>🤖 ᴍᴋᴠ ʙᴏᴛs 🤖</b></a>"""

    STICKER_TXT = """<b>● ʜᴇʟᴘ :  sᴛɪᴄᴋᴇʀ - ɪᴅ

ʏᴏᴜ ᴄᴀɴ ᴜsᴇ ᴛʜɪs ᴍᴏᴅᴜʟᴇ ᴛᴏ ғɪɴᴅ ᴀɴʏ sᴛɪᴄᴋᴇʀs ɪᴅ</b>

<b>● ᴜsᴀɢᴇ ››</b>

➪<b> ᴛᴏ ɢᴇᴛ ᴀ sᴘᴇᴄɪғɪᴄ sᴛɪᴄᴋᴇʀ ɪᴅ</b>

<b>● ᴄᴏᴍᴍᴀɴᴅs ››</b>

➪ /stickerid <b>- ʀᴇᴘʟʏ ᴛᴏ ᴀɴʏ sᴛɪᴄᴋᴇʀ</b>

<b>● ᴀʟʟ ᴄʀᴇᴅɪᴛs :</b> <a href=https://t.me/Mkv_bots><b>🤖 ᴍᴋᴠ ʙᴏᴛs 🤖</b></a>"""

    YTTHUMB_TXT = """<b>● ʜᴇʟᴘ :  ʏᴛ - ᴛʜᴜᴍʙɴᴀɪʟ 

ʜᴇʟᴘs ʏᴏᴜ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅ ᴀɴʏ ʏᴏᴜᴛᴜʙᴇ ᴠɪᴅᴇᴏ ᴛʜᴜᴍʙɴᴀɪʟ </b>

<b>● ᴄᴏᴍᴍᴀɴᴅs ››</b>

<b>ᴛʏᴘᴇ /ytthumb ᴀɴᴅ ᴠɪᴅᴇᴏ ʟɪɴᴋ</b>

<b>● ᴇxᴀᴍᴘʟᴇ  ››</b>
<code>/ytthumb https://youtube.com/channel/UCdk53DCXKoKOKgKeohUEnmA</code>

<b>● ᴀʟʟ ᴄʀᴇᴅɪᴛs :</b> <a href=https://t.me/Mkv_bots><b>🤖 ᴍᴋᴠ ʙᴏᴛs 🤖</b></a>"""

    ABOOK_TXT = """<b>● ʜᴇʟᴘ :  ᴀᴜᴅɪᴏ ʙᴏᴏᴋ </b>

<b>● ᴜsᴀɢᴇ ››</b>

<b>ʏᴏᴜ ᴄᴀɴ ᴄᴏɴᴠᴇʀᴛ ᴀ ᴘᴅғ ғɪʟᴇ ᴛᴏ ᴀ ᴀᴜᴅɪᴏ ғɪʟᴇ ᴡɪᴛʜ ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ </b>


<b>● ᴄᴏᴍᴍᴀɴᴅs ››</b>

➪ <b>/audiobook - ʀᴇᴘʟʏ ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ ᴛᴏ ᴀɴʏ ᴘᴅғ ᴛᴏ ɢᴇɴᴇʀᴀᴛᴇ ᴛʜᴇ ᴀᴜᴅɪᴏ</b>

<b>● ᴀʟʟ ᴄʀᴇᴅɪᴛs :</b> <a href=https://t.me/Mkv_bots><b>🤖 ᴍᴋᴠ ʙᴏᴛs 🤖</b></a>"""

    GTRANS_TXT = """<b>● ʜᴇʟᴘ :  ɢᴏᴏɢʟᴇ ᴛʀᴀɴsʟᴀᴛᴏʀ 

ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ ʜᴇʟᴘs ʏᴏᴜ ᴛᴏ ᴛʀᴀɴsʟᴀᴛᴇ ᴀ ᴛᴇxᴛ ᴛᴏ ᴀɴʏ ʟᴀɴɢᴜᴀɢᴇ ʏᴏᴜ ᴡᴀɴᴛ. ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ ᴡᴏʀᴋs ᴏɴ ʙᴏᴛʜ ᴘᴍ ᴀɴᴅ ɢʀᴏᴜᴘ </b>

<b>● ᴄᴏᴍᴍᴀɴᴅs ᴀɴᴅ ᴜsᴀɢᴇ ››</b>

➪<b> /tr - ᴛᴏ ᴛʀᴀɴsʟᴀᴛᴇ ᴛᴇxᴛs ᴛᴏ ᴀ sᴘᴇᴄɪғɪᴄ ʟᴀɴɢᴜᴀɢᴇ</b> 

<b>● ɴᴏᴛᴇ ››</b>

<b>ᴡʜɪʟᴇ ᴜsɪɴɢ /tr ʏᴏᴜ sʜᴏᴜʟᴅ sᴘᴇᴄɪғʏ ᴛʜᴇ ʟᴀɴɢᴜᴀɢᴇ ᴄᴏᴅᴇ</b>

<b>● ᴇxᴀᴍᴘʟᴇ  ››</b>

<b>➪ ᴇɴ = ᴇɴɢʟɪsʜ 
➪ ᴍʟ = ᴍᴀʟᴀʏᴀʟᴀᴍ
➪ ᴛᴀᴍ = ᴛᴀᴍɪʟ</b>

<b>● ᴀʟʟ ᴄʀᴇᴅɪᴛs :</b> <a href=https://t.me/Mkv_bots><b>🤖 ᴍᴋᴠ ʙᴏᴛs 🤖</b></a>"""

    RESTRIC_TXT = """<b>● ᴄᴏᴍᴍᴀɴᴅs ᴀɴᴅ ᴜsᴀɢᴇ ››</b>

➪ <b>/ban - ᴛᴏ ʙᴀɴ ᴀ ᴜsᴇʀ ғʀᴏᴍ ᴛʜᴇ ɢʀᴏᴜᴘ </b>
➪ <b>/unban - ᴛᴏ ᴜɴʙᴀɴ ᴀ ᴜsᴇʀ ғʀᴏᴍ ᴛʜᴇ ɢʀᴏᴜᴘ  </b>
➪ <b>/tban - ᴛᴏ ᴛᴇᴍᴘᴏʀᴀʀɪʟʏ ʙᴀɴ ᴀ ᴜsᴇʀ </b>
➪<b> /mute - ᴛᴏ ᴍᴜᴛᴇ ᴀ ᴜsᴇʀ ɪɴ ᴛʜᴇ ɢʀᴏᴜᴘ </b>
➪<b>/unmute - ᴛᴏ ᴜɴᴍᴜᴛᴇ ᴀ ᴜsᴇʀ ɪɴ ᴛʜᴇ ɢʀᴏᴜᴘ </b>
➪<b>/tmute - ᴛᴏ ᴛᴇᴍᴘᴏʀᴀʀɪʟʏ ᴍᴜᴛᴇ ᴀ ᴜsᴇʀ </b>

<b>● ɴᴏᴛᴇ ››</b>

<b>ᴡʜɪʟᴇ ᴜsɪɴɢ</b> /tmute <b>ᴏʀ</b> /tban <b>you should specify the time limit.</b>

<b>● ᴇxᴀᴍᴘʟᴇ  ››</b>
/tban <b>2ᴅ ᴏʀ</b> /tmute <b>2ᴅ.</b>
<b>ʏᴏᴜ ᴄᴀɴ ᴜsᴇ ᴠᴀʟᴜᴇs - ᴍ/ʜ/ᴅ
      ➪ ᴍ = ᴍɪɴᴜᴛᴇs
      ➪ ʜ = ʜᴏᴜʀs
      ➪ ᴅ = ᴅᴀʏs </b>

<b>● ᴀʟʟ ᴄʀᴇᴅɪᴛs :</b> <a href=https://t.me/Mkv_bots><b>🤖 ᴍᴋᴠ ʙᴏᴛs 🤖</b></a>"""
    CREATOR_REQUIRED = """❗<b>ʏᴏᴜ ʜᴀᴠᴇ ᴛᴏ ʙᴇ ᴛʜᴇ ɢʀᴏᴜᴘ ᴄʀᴇᴀᴛᴏʀ ᴛᴏ ᴅᴏ ᴛʜᴀᴛ.</b>"""
      
    INPUT_REQUIRED = "❗ **ᴀʀɢᴜᴍᴇɴᴛs ʀᴇϙᴜɪʀᴇᴅ**"
      
    KICKED = """✔️ **sᴜᴄᴄᴇssғᴜʟʟʏ ᴋɪᴄᴋᴇᴅ** {} **ᴍᴇᴍʙᴇʀs ᴀᴄᴄᴏʀᴅɪɴɢ ᴛᴏ ᴛʜᴇ ᴀʀɢᴜᴍᴇɴᴛs ᴘʀᴏᴠɪᴅᴇᴅ.**"""
      
    START_KICK = """🚮 **ʀᴇᴍᴏᴠɪɴɢ ɪɴᴀᴄᴛɪᴠᴇ ᴍᴇᴍʙᴇʀs ᴛʜɪs ᴍᴀʏ ᴛᴀᴋᴇ ᴀ ᴡʜɪʟᴇ...**"""
      
    ADMIN_REQUIRED = """❗<b>എന്നെ Admin ആക്കത്ത സ്ഥലത്ത് ഞാൻ നിക്കില്ല പോകുവാ Bii..Add Me Again with all admin rights.</b>"""
      
    DKICK = """✔️ **ᴋɪᴄᴋᴇᴅ** {} **Deleted Accounts Successfully.**"""
      
    FETCHING_INFO = """<b>ഇപ്പൊ എല്ലാം അടിച്ചുമാറ്റി തരാം...</b>"""
      
    STATUS = """{}\n<b>Chat Member Status</b>**\n\n```<i>Recently``` - {}\n```Within Week``` - {}\n```Within Month``` - {}\n```Long Time Ago``` - {}\nDeleted Account - {}\nBot - {}\nUnCached - {}</i>
"""
